...
hist_inds = np.arange(0, len(df))[(df['event_type'] == 'le') & (df['evid'] < 60000000)]

percent_test_hist = 0.16
...
f = h5py.File("uuss_test_fewerhist.h5", "w")
f.create_dataset("X", data=X_smaller)
f.create_dataset("Y", data=Y_smaller)
f.close()

df_smaller.to_csv("uuss_test_fewerhist.csv")
# %%
