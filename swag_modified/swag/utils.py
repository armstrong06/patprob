...
def train_epoch(
    ...
    for i, (input, target) in enumerate(loader):
        ...
        if not regression:
            ...
            # Added by Alysha to try and track the residuals
            pred = output.data
            resids = pred.sub(target.data.view_as(pred))
            residual_std += resids.sum().item()
            residual_mean += resids.std().item()

        num_objects_current += input.size(0)

        if verbose and 10 * (i + 1) / num_batches >= verb_stage + 1:
            print(
                "Stage %d/10. Loss: %12.4f. Acc: %6.2f. Resid Mean: %8.4f. Resid STD: %8.4f."
                % (
                    verb_stage + 1,
                    loss_sum / num_objects_current,
                    correct / num_objects_current * 100.0,
                    residual_mean / num_objects_current, 
                    residual_std / num_objects_current
                )
            )
            verb_stage += 1
